...
- Fallback smoke execution keeps one run-directory fd and one lock-file fd, snapshots its command and state version with the lock held, unlocks that same fd for a hard-time/byte-bounded process-group run, and CAS-merges only its result after reacquiring it. Nonzero helper results take precedence over any `READY` output. One deferred `SIGHUP`/`SIGTERM`/`SIGQUIT` transaction remains armed across the start-new-session helper, proves bounded process-group kill/drain/reap, restores the original handlers/mask, and only then redelivers the first signal. Phase and child-terminal updates remain available while a smoke is hung; lock replacement or run object/access drift fails closed.
...
- Local full suite: `103` tests passed.
- Focused runner suite: `27` tests passed, including deterministic lock interleavings, one-descriptor smoke snapshot/merge, `READY`-then-failure classification, bounded timeout/byte/residual-process cleanup, deferred terminal-signal process-group cleanup and redelivery, state identity/digest CAS classifications, and published-temp identity binding.
