...
- The Stop hook validates exact current-repo containment, exact `state.repo_root`, no-symlink path components, regular state/prompt files, current-user ownership, mode `0700`, and stable run object/access identity before and after automatic prompt refresh. It keeps the original run fd open, carries device/inode plus uid/gid/mode through the bridge, and also passes a complete expected compatibility-runner version that the runner checks before writing. Refresh JSON returns schema-bound runner and prompt versions; the adapter compares expected/returned/current runner state and rereads each named prompt along the pinned run fd, binding object/access/size plus two bounded byte reads and SHA-256 digests. Metadata-only timestamp churn remains benign, while external, access-changed, content-changed, symlink-backed, or regular-file/directory replacement records fail closed before a prompt path is rendered.
...
- Local full suite: `111` tests passed.
- Focused runner suite: `31` tests passed, including deterministic lock interleavings, one-descriptor smoke snapshot/merge, stdout/stderr `READY`-then-failure classification, selector-construction failure before launch, bounded timeout/byte/residual-process cleanup, default and returning-handler terminal-signal process-group cleanup and redelivery, state identity/access/digest CAS classifications, published-temp object/access binding through the final read, and expected-runner mismatch rejection before prompt/state writes.
- Focused hook-adapter suite: `52` tests passed, including external run, repo mismatch, run/state/prompt symlink, post-refresh child/parent regular-file replacement, prompt and runner content/access-policy drift, metadata-only timestamp churn, and exact returned-version fail-closed checks.
