...
- `prepare`: build and validate the complete initial state, its worst-case terminal-capacity projection, and every UTF-8 artifact before opening or creating the run path; then create `.codex-tmp/waited-delivery/<run-id>/`, descriptor-revalidate and parent-directory-fsync each name entry, and publish `state.json`, `child-contract.md`, `child-prompt.md`, `parent-prompt.md`, and fallback-smoke artifacts. Adapter-launched state uses schema `4` and records the stable preparation UUID. Any `--phase` override must retain the required `internal_review` phase. Use `--json` when a future supervisor or hook needs machine-readable artifact paths.
...
- `prepare-active-run`: outer-adapter command that resolves an unambiguous observed session, descriptor-validates any existing indexed run before accepting its terminal state, persists a schema `2` `preparing` reservation plus inherited preparation lease before `prepare-live`, and CAS-promotes only an exact schema `4` run-state attestation to `active`; after bridge failure it retires the adapter's inherited lease reference once and requires an independent nonblocking reacquisition before inspecting or clearing an absent run entry. Prefer `--session-id` when available, let host-injected `CODEX_THREAD_ID` act as the default explicit parent-session selector when present, and otherwise use `--transcript-path` or `--prompt-text` as explicit recovery selectors instead of trusting repo-global recency.
- `recover-active-run`: inspect the exact preparation (`doctor`), promote a complete same-transaction run (`resume`), or clear a descriptor-proven absent reservation only after acquiring the inherited lease (`clear-absent`). It never follows a returned foreign path or recursively deletes a partial run.
