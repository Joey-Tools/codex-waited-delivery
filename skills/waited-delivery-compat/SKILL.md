...
- `refresh-prompts`: runner recovery command that rewrites `child-prompt.md` and `parent-prompt.md` with commands bound to the currently loaded compatibility runner and rebinds their canonical run-local artifact paths. It holds the same run-level lock as phase and child-terminal updates, rejects symlink/non-regular state or prompt artifacts, and commits prompt/state changes with descriptor-relative atomic replacement. An optional complete expected-runner descriptor is checked before any prompt/state write and again around publication. State loads return the opened file's device/inode, POSIX access policy, size, and content digest; the subsequent save compares that original version and distinguishes missing, replaced, access-changed, content-changed, and unreadable state. Publication also proves that the final named file is the fsynced temporary inode with the intended bytes, and JSON refresh results return the exact runner and prompt descriptors.
- `refresh-prompts-live`: bridge command that wraps `refresh-prompts --json`; the explicitly enabled stop hook uses it before returning continuation guidance for an older active run and carries complete expected-runner plus initially validated run-directory descriptors into the runner, then returns the refresh schema and exact runner/child/parent versions for post-refresh comparison.
...
- `stop-hook`: compatibility hook entrypoint that is inert unless passed `--enable-compat-hook`; when explicitly enabled, it first proves that the indexed run is a direct no-symlink child of the current repo's `.codex-tmp/waited-delivery`, that `state.repo_root` exactly matches that repo, and that state/prompt artifacts are regular no-follow files. It keeps that validated run-directory descriptor open throughout bridge refresh, binds its device/inode and POSIX owner/group/mode, and reads each artifact twice with a bounded byte comparison while binding its named entry, descriptor, access policy, size, and content digest. It also snapshots the compatibility runner, carries that expected version through the bridge, and compares the returned and current runner versions before rendering any persisted prompt path. Benign timestamp churn is not content mutation. It regenerates both persisted prompts through the loaded compatibility runner so historical absolute runner paths are not reused and blocks premature finish with a continuation prompt. An external, access-changed, content-changed, replaced, or linked run fails closed without refreshing it.
