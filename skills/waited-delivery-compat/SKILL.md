...
- `prepare`: build and validate the complete initial state, its worst-case terminal-capacity projection, and every UTF-8 artifact before opening or creating the run path. The projection reserves a maximum encoded `child_session_id`, `child_started_at`, bounded late-bound parent metadata, and every terminal transition field; child IDs are limited to `1024` UTF-8 bytes and `6146` JSON-value bytes. Then create `.codex-tmp/waited-delivery/<run-id>/`, descriptor-revalidate and parent-directory-fsync each name entry, and publish `state.json`, `child-contract.md`, `child-prompt.md`, `parent-prompt.md`, and fallback-smoke artifacts. Adapter-launched state uses schema `5` and records the stable preparation UUID. When schema `1` through `4` contains an already-oversized child ID or parent metadata value, schema `5` records its exact SHA-256/UTF-8/JSON identity: that unchanged value remains usable through save, finish, reconciliation, and finalization, but new or changed oversized text is rejected. Any `--phase` override must retain the required `internal_review` phase. Use `--json` when a future supervisor or hook needs machine-readable artifact paths.
- `prepare-live`: bridge command that wraps `prepare --json`, injects parent metadata from args or the bridge env contract, and for adapter preparations inherits and forwards the owner-private preparation-lease descriptor together with the stable preparation UUID.
...
- `user-prompt-submit-hook`: compatibility hook entrypoint that is inert unless passed `--enable-compat-hook`; when explicitly enabled, it records the current session metadata into a repo-local adapter index. Each index load/mutation/commit uses a descriptor-bound transaction; `prepare-active-run` deliberately uses two short transactions around bridge work so Stop and recovery can observe the durable intermediate reservation. The repository root, `.codex-tmp`, adapter directory, index, lock, preparation lease, runs root, run, state/prompts, and bridge/runner source parents and files must be current-user-owned, reject group/other write, and carry no Darwin extended or Linux POSIX ACL. Held descriptors revalidate that access policy at the relevant commit or launch boundary. Exact `0700` directories and `0600` no-follow lock/index/lease files prevent symlink escape, umask disclosure, partial JSON publication, and lost cooperative read-modify-write updates.
- `stop-hook`: compatibility hook entrypoint that is inert unless passed `--enable-compat-hook`; when explicitly enabled, it first proves that the indexed run is a direct no-symlink child of the current repo's `.codex-tmp/waited-delivery`, that `state.repo_root` exactly matches that repo, and that state/prompt artifacts are regular no-follow files. It keeps that validated run-directory descriptor open throughout refresh, binds its device/inode and POSIX owner/group/mode, and reads each artifact twice with a bounded byte comparison while binding its named entry, descriptor, access policy, size, and content digest. A current-user legacy directory with exact mode `0755` is tightened through that open descriptor to `0700`, with device/inode and owner/group revalidated; every other non-`0700` policy fails closed. All adapter-to-bridge paths—prepare, parent binding, child attachment, child finish, parent reconciliation, and prompt refresh—stable-bind the bridge and runner source objects and exact bytes, revalidate the named versions, and send a bounded two-source frame through anonymous stdin to a fixed Python `-I -B -S -c` bootstrap with Python injection variables removed. That bootstrap requires exact framing and EOF before compiling the bridge in memory; the bridge sends the already bound runner bytes through a second framed pipe to another isolated bootstrap. No adapter-driven path executes or reopens either source path. No source snapshot file or inherited source FD exists. Prompt-refresh schema `3` receipts bind both source-provenance versions, both `anonymous-pipe-memory` transports, non-reopenability, canonical compile filenames, and isolated execution. Each adapter bridge launch uses a new session, a hard timeout, a captured-byte ceiling, bounded whole-process-group cleanup, and deferred `SIGHUP`/`SIGTERM`/`SIGQUIT` redelivery where applicable. Its leader remains unreaped while the group can still be signalled: Linux observes exit with `waitid(..., WNOWAIT)`, Darwin with `kqueue` `NOTE_EXIT`, and reap occurs only after output pipes drain and the group has no live members. Darwin uses `libproc` to distinguish live, zombie-only, and absent groups; unknown evidence fails closed. Early input rejection is terminal `126`, and the adapter closes the source writer before cleanup so a child or descendant cannot turn an incomplete frame into success. The runner revalidates the injected runner bytes under the run lock immediately before any prompt/state write. Source-path replacement, content drift, or access-policy drift observed before launch therefore fails before any prompt/state write; an A→B→A change after stable binding cannot execute B because no source path is reopened. Benign timestamp churn is not content mutation. It regenerates both persisted prompts with the canonical runner path and blocks premature finish with a continuation prompt. An external, access-changed, content-changed, replaced, or linked run fails closed without refreshing it.
- `prepare-active-run`: outer-adapter command that resolves an unambiguous observed session, descriptor-validates any existing indexed run before accepting its terminal state, persists a schema `3` `preparing` reservation plus inherited preparation lease before `prepare-live`, and CAS-promotes an exact preparation-aware schema `4` or `5` run-state attestation to `active`; after bridge failure it retires the adapter's inherited lease reference once and requires an independent nonblocking reacquisition before inspecting an absent run entry. A proven quiescent absence first CAS-enters `cleanup_pending` with the complete reservation intact, then unlinks the descriptor-bound lease and fsyncs its parent, and only then final-CAS-publishes `cleanup_complete` while retaining the full exact reservation identity. Schema-v2 readers reject schema `3` before mutation, so they cannot rewrite a cleanup fence as active. Prefer `--session-id` when available, let host-injected `CODEX_THREAD_ID` act as the default explicit parent-session selector when present, and otherwise use `--transcript-path` or `--prompt-text` as explicit recovery selectors instead of trusting repo-global recency.
- `recover-active-run`: inspect the exact preparation (`doctor`), promote a complete same-transaction run (`resume`), or finish a descriptor-proven absent cleanup (`clear-absent`). `clear-absent` handles `cleanup_pending` with its exact lease still present, the crash state where the lease is already absent but the final index CAS remains, and an already-persisted `cleanup_complete` tombstone. The absent-lease path fsyncs and revalidates the adapter parent before that final CAS; every actual final publication retains the exact preparation/run/lease identity, so replace/fsync/readback/revalidation uncertainty can converge on retry from disk. A new preparation may exact-CAS-supersede a verified tombstone, while an old retry cannot clear the new reservation. It never clears identity before durable lease unlink, follows a returned foreign path, or recursively deletes a partial run.
...
The source and prompt transport contract protects content stability after bytes have been stable-bound: a same-UID filesystem attacker, including one holding a writable descriptor or hard link to the original regular file, cannot change the bytes later compiled or offered for delivery. Legacy source, every adapter-to-bridge command, and fallback-prompt delivery use anonymous pipes with no writable regular-file snapshot; fixed bootstraps additionally require bounded exact lengths, EOF, and SHA-256 before compiling source bytes in memory. The adapter compiles its stable-bound bridge bytes under Python `-I -B -S -c`, and that bridge compiles the stable-bound runner bytes through a second isolated frame; neither layer path-executes after binding. Source-path object identity, owner/access policy, size, and digest remain separately bound provenance and are revalidated before launch. Both adapter and ordinary runner reopen require current-user ownership, reject group/other write, and reject any Darwin extended or Linux POSIX ACL on the repository root, `.codex-tmp`, adapter/index/lock/lease objects, runs root, run, state/prompts, bridge/runner source parents and files, smoke prompt, and run lock as applicable. Only an exact current-user legacy `0755` run is tightened through its held descriptor to `0700`, before any artifact read. Run directories, state, prompts, and the adapter index retain their descriptor-relative identity/access/content contracts. Runner, adapter, and test supervisors keep the process-group leader unreaped until pipes drain and live group membership is absent; Linux uses `waitid(..., WNOWAIT)`, Darwin uses `kqueue` `NOTE_EXIT` plus `libproc`, and unknown group evidence fails closed. This prevents cleanup from signalling a recycled numeric PGID. The fallback supervisor proves complete pipe delivery when possible and rejects an observed early reader close, but it does not attest semantic prompt consumption after the kernel accepts all bytes. This does not claim protection against `ptrace`, debugger or process-memory injection, or arbitrary code already executing inside the trusted process. Replacement, unreadable state, incomplete delivery, or any unprovable selected property fails closed; the legacy redirect, compatibility runner, bridge, and hook adapter implement their portions without runtime imports from one another.
