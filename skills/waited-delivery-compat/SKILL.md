...
- `run-fallback-smoke`: open one run-directory descriptor and one lock-file descriptor, snapshot the smoke command and state version while that lock is held, unlock it while executing the command with a hard deadline, captured-byte ceiling, and process-group cleanup, then reacquire the same lock descriptor and CAS-merge only the smoke result. Any nonzero process result wins over a printed `READY`. While the start-new-session helper is live, defer `SIGHUP`, `SIGTERM`, and `SIGQUIT`, kill/drain/reap its process group, restore the caller's handlers and signal mask, and only then redeliver the first signal with its original terminal semantics. Phase and child-terminal updates can therefore complete while the smoke is still running; lock replacement, run identity/access drift, or a concurrent smoke-state change blocks the merge instead of being overwritten.
