...
- `stop-hook`: compatibility hook entrypoint that is inert unless passed `--enable-compat-hook`; when explicitly enabled, it first proves that the indexed run is a direct no-symlink child of the current repo's `.codex-tmp/waited-delivery`, that `state.repo_root` exactly matches that repo, and that state/prompt artifacts are regular no-follow files. It then regenerates both persisted prompts through the loaded compatibility runner so historical absolute runner paths are not reused and blocks premature finish with a continuation prompt. An external, replaced, or linked run fails closed without refreshing it.
