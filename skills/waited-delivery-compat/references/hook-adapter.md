...
- `Stop` keeps that descriptor open while the bridge runs, sends its full recorded identity plus a descriptor-safe compatibility-runner snapshot through the bridge, and rereads state/prompts through the same descriptor before rendering guidance. The runner returns one version for itself and each newly published prompt; the adapter requires the refresh schema, compares the expected/returned/current runner, and binds each prompt's named entry, object identity, uid/gid/mode, size, and two bounded byte reads plus SHA-256 digest. Timestamp-only churn is deliberately excluded because it changes none of those protected properties. A symlink, regular-file or directory replacement, owner/group drift, mode change, or content change therefore fails closed before a prompt path is rendered.
